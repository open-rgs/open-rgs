 ...
 *
 *  Math returns a multiplier and ops. Core multiplies multiplier × bet to
...
export interface GameMode {
   ...
   *  single round's win at 10,000 × bet. Most jurisdictions require this.
...
export interface SettleSimple {
   ...
   *  Includes priceMultiplier × stakeMultiplier already. */
  bet: number;
  /** Index into the session's allowed_bets ladder — what the player picked. */
  betIndex: number;
  /** Multiplier applied to the chosen bet level (mode stakeMultiplier × priceMultiplier). */
