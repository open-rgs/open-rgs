...
export function createOrchestrator(cfg: OrchestratorConfig): OrchestratorAPI {
  ...
  function computeBet(
    ...
    // priceMultiplier × stake) and computes its own debit at full
  ...
  async function spin(req: ClientRequestSpin, conn: ConnectionMeta): Promise<ClientResponseSpin> {
    ...
    // Win = multiplier × what was actually paid. For ante/buy modes
 ...
 *  winning multiplier would settle `win = multiplier × 0 = 0`, silently
