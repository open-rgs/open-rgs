...
3. Validate the resolved mode is `kind: "simple"`. Otherwise
   `INVALID_MODE`.
4. Compute bet: `allowedBets[betIndex] × priceMultiplier × stakeMultiplier`.
...
```
bet = allowedBets[betIndex] × priceMultiplier × stakeMultiplier
```
...
The orchestrator passes `betIndex` and `priceMultiplier × stakeMultiplier`
