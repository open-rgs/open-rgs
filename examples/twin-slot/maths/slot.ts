// twin-slot: the SAME minimal SIMPLE-round slot as maths/slot.lua and
...
// Keep this ladder, slot.lua's, and slot.zig's decide() in lock-step - same
...
// `host.rng_next()` exactly as the Lua twin draws through `host.rng_next()`
// and the Zig twin imports `host.rng_next` - one auditable source in all three
